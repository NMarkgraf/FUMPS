...
Unter einer (**univariaten**) **Funktionen** (oder **Abbilung**), 
also Funktionen in einer Variabeln, verstehen wir:
...
1. $\displaystyle (x^3-3x^2+3x-1):(x-1)$\setlength{\itemsep}{12pt}

2. $\displaystyle (x^3+3x^2+3x+1):(x-1)$

3. $\displaystyle (x^4-5x^3+2x-1):(x^2-2x+1)$

4. $\displaystyle (-x^5+x^4-x^3+x^2-x+1):(2x^2-2x+1)$
...
- $-2(x-2)^2+2 = -2x^2 + 8x - 6$\setlength{\itemsep}{12pt}
...
- $d(-1, 1) = |-1 - 1| = |-2|= 2$\setlength{\itemsep}{12pt}
- $d(0, 5) = |0 - 5| = |-5| = 5$
- $d(5, 1) = |5 - 1| = |4| = 4$ 
...
Aus $|a| = b$ folgt für reelle Zahlen $>a = b$ oder $a = -b$. 
